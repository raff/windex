...
- Local persistence in `localStorage` for layout and page metadata (title/subtitle, grid size, and themes).
- Configuration panel with a live table of current widgets and a theme selector.
- Theme system powered by `meta.themes` and `meta.theme` (light/dark plus custom themes).
- Import/Export:
...
4. Click the page title to edit the dashboard title/subtitle.
